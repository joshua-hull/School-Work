...
@ CPSC 2311-002 Lab 12: Circular Buffer
	.syntax unified
	.arch armv7-a
	.eabi_attribute 27, 3
	.fpu vfpv3-d16
...
printbuf:
	...
	ldr	r4, .L8             @ load the address of "count" into r7
...
put:
	...
	ldr     r5, .L12
    ldr 	r5, [r5] 		@ r5 = count
	cmp 	r5, #8			@ count == CBUFSIZE
	beq 	put_full		@ if(count == CBUFSIZE)

	
    ldr     r5, .L12
    ldr 	r1, [r5, #8] 	@ r1 = in
    ldr     r5, [r5, #12]
	strb 	r0, [r5, r1]	@ cbuf[in] = val

	mov 	r2, #7
	add 	r1, r1, #1 		@ in = in +1
	and 	r1, r1, r2

	str 	r1, [r5, #8] 	@ in = in & CBUFMASK
...
get:
	...
	ldr 	r4, [r4] 	@ r4 = count
	cmp 	r4, #0 		@ count == 0
	beq		get_empty 	@ if count == 0

						@ else...
	
	ldr 	r0, [r4, #8]	@ r0 = out
	ldrb 	r1, [r4, r0]	@ r1 = cbuf[out] = val

	add 	r0, r0, #1 		@ out = out +1
	mov 	r2, #7 			@ r2 = CBUFFMASK
	and		r0, r0, r2		@ out = out & CBUFFMASK

...
/** outImage.c
    ...
    Prototype:
       void outImage(char *fileName, image_t *image);

    where
       fileName:    pointer to name of output file to create,
       image: pointer to the image_t of the image to write.
...
void outImage(char *fileName, image_t *image) {
   ...
   fprintf(outFile, "P6 %d %d %d\n",
     image->columns, image->rows, image->brightness);
   fwrite(image->image, sizeof(pixel_t),
            image->columns * image->rows, outFile);
}

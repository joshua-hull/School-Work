...
tuple_t cross(tuple_t t1, tuple_t t2) {
    ...
    // Compute the compontents of the cross product of tw 3d vectors
    tuple_t returnTuple;
    returnTuple.x = t1.y*t2.z - t1.z*t2.y;
    returnTuple.y = t1.z*t2.x - t1.x*t2.z;
    returnTuple.z = t1.x*t2.y - t1.y*t2.x;
    return(returnTuple);
}

...
int main(int argc, char **argv) {
	...
	for(k=1; k<= iterations; k++){
	    ...
	    my_exchange = MPI_Wtime() - my_exchange;
	...
	MPI_Reduce(&timer, &average, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	MPI_Reduce(&exchange, &exchange_total, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    ...
    if(rank == 0){
	    ...
	    fprintf(fp, "P3\n%d %d\n15\n",dim, dim);
	    fclose(fp);
	    printf("compute_time: %.10f\n", average/size);
	    printf("comm_time: %.10f\n", exchange_total/size);
    }
